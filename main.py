...
hyperion_host = '192.168.1.177'
hyperion_port = 19444

# Change this according to your led configuration.
horizontal_led_num = 30
vertical_led_num = 30
...
def run_effect(my_effect):
    ...
    import json
    import sys
    with open('effects/' + my_effect + '.json', 'r') as f:
        effects_data = json.load(f)
    
    hyperion.args = effects_data['args']
    print "Running effect: " + effects_data['name']
    #runpy.run_module('effect')
    runpy.run_path('effects/' + effects_data['script'])
...
def main(my_effect):
    ...
    effect_thread = Thread(target=run_effect, args=[my_effect])
